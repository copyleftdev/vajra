...
fn cmd_governance(input: &str, author_field: &str, time_field: &str, cli: &Cli) -> Result<()> {
    ...
    let records = match parsed.as_array() {
        Some(arr) => arr.clone(),
        None => {
            anyhow::bail!("governance command expects a JSON array of records");
        }
    };

    let report = vajra_stats::governance_analysis(&records, author_field, time_field)
    ...
    match cli.format {
        ...
        Format::Text => {
            let t = vajra_stats::governance_text(&report);
            let t = maybe_redact(&t, cli);
            print!("{t}");
        }
        Format::Markdown => {
            let m = vajra_stats::governance_markdown(&report);
            let m = maybe_redact(&m, cli);
            print!("{m}");
        }
        Format::CompactAi => {
            let j = serde_json::to_string(&report).context("JSON serialization failed")?;
            let j = maybe_redact(&j, cli);
            println!("{j}");
        }
    }
